...
- 保留辅助数据库的广义发现能力，确保供应商同步、幽灵任务清理和跨库删除行为不受影响。

## 1.2.38 - 2026-07-14

- 修复检查更新和安装包下载强制关闭系统代理导致的 GitHub 请求失败。
- 公网更新请求默认使用系统代理，发送失败时自动直连重试，本地回环地址保持直连。
- 检查更新失败时显示完整的底层错误原因，便于区分代理、DNS、TLS 和超时问题。

## 1.2.37 - 2026-07-14

- 修复桥接重注入时本地数据服务丢失的问题，避免会话删除、导出等本地操作失效。
- 移除概览页推广卡片、推荐内容页面和独立的中转站环境检测页面，保留供应商环境冲突提示与清理能力。
- 优化基础设置布局，统一界面主题与供应商测试模型的设置行，并改善窄屏显示。
- 支持推送 `v*` 标签后自动创建 GitHub Release，并上传 Windows、macOS 安装包和 `latest.json`。
