...
const FALSE_AFTER_PROVIDER_SWITCH_KEYS: &[&str] =
...
fn normalize_current_state(state: &mut Map<String, Value>, changed: &mut BTreeSet<String>) {
    ...
    for key in FALSE_AFTER_PROVIDER_SWITCH_KEYS {
        if state.get(*key).is_some() {
            replace_if_changed(state, key, Value::Bool(false), changed);
        }
    }
...
fn is_safe_atom_key(key: &str) -> bool {
    SAFE_ATOM_KEYS.contains(&key)
        ...
        || key.starts_with("fast-mode-")
        || key.starts_with("first-awake-pet-notification-")
        || key.starts_with("has-seen-fast-mode-")
        || key.starts_with("has-opened-plugin-")
        || key.starts_with("has-opened-skill-")
        || key.starts_with("plugin-")
        || key.starts_with("codex-plugin-")
        || key.starts_with("openai-mcp-")
